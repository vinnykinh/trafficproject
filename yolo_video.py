...
np.random.seed(42)
COLORS = np.random.randint(0, 255, size=(len(list_of_vehicles), 3),
	dtype="uint8")
...
def initializeVideoWriter(video_width, video_height, videoStream):
	...
	return cv2.VideoWriter("", fourcc, sourceVideofps,
...
# frame dimensions
videoStream = cv2.VideoCapture('sample_videos/lane.mkv')
...
while True:
	...
	net.setInput(blob)
	start = time.time()
	layerOutputs = net.forward(previous_frame_detections)
	end = time.time()
